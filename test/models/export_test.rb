...
class ExportTest < ActiveSupport::TestCase
  ...
  test "exports scheduled newsletter selection for scheduled articles" do
    slug = "export-scheduled-newsletter-#{SecureRandom.hex(6)}"
    article = Article.create!(
      title: "Scheduled Newsletter Export",
      slug: slug,
      status: :schedule,
      scheduled_at: 1.day.from_now,
      content_type: :html,
    ...
    Zip::File.open(exporter.zip_path) do |zip|
      ...
      assert_equal "true", exported["scheduled_send_newsletter"]
    end
  ensure
    File.delete(exporter.zip_path) if exporter&.zip_path.present? && File.exist?(exporter.zip_path)
    article&.destroy
